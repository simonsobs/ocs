...
def test_block_append_influxdb_tags():
    """Test adding some data to a Block with changing influxdb tags. The tags
    should not change when no tags are provided in the second append().
    ...
    # Add new samples without tags
    time_sample = 1558044485.2398098
    data_sample = 4
    ...
    assert test_block.tags == tags


def test_block_extend_influxdb_tags():
    """Test extending data in a Block with changing influxdb tags. The tags
    should not change when no tags are provided in the second extend().
    ...
    tags = {'key1': {'key': 1, '_field': 'value'}},
    data = {'timestamps': time_samples,
            'influxdb_tags': tags,
            'data': {'key1': data_samples}}
    test_block.extend(data)

    assert test_block.data['key1'] == data_samples
    assert test_block.timestamps == time_samples

    # Add new samples without tags
    time_samples = [1558044485.2398098, 1558044486.2398098,
                    1558044487.2398098]
    data_samples = [4, 5, 6]
